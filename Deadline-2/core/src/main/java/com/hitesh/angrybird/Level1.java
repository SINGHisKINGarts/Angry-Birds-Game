...
public class Level1 implements Screen {
    ...
    private Texture redBirdTexture, yellowBirdTexture, backgroundTexture, pigTexture, slingTexture;
...
//    private List<Block> blocks;
    ...
    private void resetLevel() {
        ...
        // Initialize bird queue with exactly 2 birds
        ...
        pigs = new ArrayList<>();
        pigs.add(new Vector2(6.25f, 2.84f));
        pigs.add(new Vector2(7.25f, 1.9f));
        pigs.add(new Vector2(9.25f, 1.85f));
    }
    ...
    public void render(float delta) {
        ...
        // Draw bird only if it's in a valid position
        if (birdLaunched && birdPosition.y > -1) {
            spriteBatch.draw(currentBirdTexture,
                    birdPosition.x - birdRadius,
                    birdPosition.y - birdRadius,
        ...
        if (pigs.isEmpty()) {
            ...
            game.setScreen(new WinScreen(game));
                ...
                remainingBirds <= 0 &&
                !game.getScreen().getClass().equals(LoseScreen.class)) {
            gameOver = true;
            game.setScreen(new LoseScreen(game));
        }
    }
    ...
    private void checkGameOver() {
        ...
        // Check if current bird is at rest and no more pigs can be hit
        if (birdLaunched &&
                birdPosition.y <= GROUND_LEVEL + birdRadius &&
                Math.abs(birdVelocity.x) < minVelocity &&
                Math.abs(birdVelocity.y) < minVelocity) {

            // Try to reset bird if possible
            resetBird();
        }

        // Check if all birds have been launched and no pigs remain
        if (remainingBirds <= 0 && pigs.size() > 0) {
            gameOver = true;
            game.setScreen(new LoseScreen(game));
        }
    }
