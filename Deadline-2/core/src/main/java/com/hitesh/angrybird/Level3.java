...
public class Level3 implements Screen {
    ...
    private Texture blackBirdTexture, yellowBirdTexture, backgroundTexture, pigTexture, slingTexture;
    ...
    private boolean LoseScreenShown=false;
//    private List<Block> blocks;
    ...
    public void show() {
        ...
        yellowBirdTexture = new Texture("Yellow Bird1.png");
        backgroundTexture = new Texture("Background.jpeg");
        pigTexture = new Texture("Pig3.png");
        slingTexture = new Texture("Catapult.png");
...
// Initialize blocks in a pyramid-like structure with variety
        blocks = new ArrayList<>();
        blocks.add(new WoodenBlock(7, 0.69f));
        blocks.add(new WoodenBlock(7, 1.665f));
        blocks.add(new WoodenBlock(8, 0.69f));
        blocks.add(new GlassBlock(7.7f, 1));
//        blocks.add(new WoodenBlock(6, 0.69f));
        blocks.add(new SteelBlock(6.84f, 1.07f));
        blocks.add(new WoodenBlock(7.97f, 1.7f));
//        blocks.add(new SteelBlock(8, 2.76f));
//        blocks.add(new GlassBlock(6.75f, 2.76f));
        blocks.add(new WoodenBlock(7.48f, 1.16f));

// Initialize bird queue with textures
        birdQueue = new LinkedList<>();
        birdQueue.add(blackBirdTexture);
        birdQueue.add(yellowBirdTexture);
        birdQueue.add(yellowBirdTexture);
//        birdQueue.add(whiteBirdTexture); // Adding a third bird for more variety
        currentBirdTexture = birdQueue.poll();
        remainingBirds = birdQueue.size();

// Initialize pigs in a staggered pattern
        pigs = new ArrayList<>();
        pigs.add(new Vector2(7.75f, 0.89f)); // Top pig
//        pigs.add(new Vector2(6.5f, 3.45f)); // Left pig
        pigs.add(new Vector2(8.2f, 2.4f)); // Right pig
        pigs.add(new Vector2(7.23f, 2.36f)); // Center pig
    ...
    public void render(float delta) {
        ...
        if (isDragging && !birdLaunched) {
            ...
            // Calculate sling attachment points
            float leftX = slingStart.x - 0.15f;
            float rightX = slingStart.x + 0.15f;
            float attachY = slingStart.y - 0.1f;

            // Draw both bands of the slingshot
            shapeRenderer.line(leftX, attachY, birdPosition.x, birdPosition.y);
            shapeRenderer.line(rightX, attachY, birdPosition.x, birdPosition.y);
            shapeRenderer.end();
        ...
        if (pigs.isEmpty()) {
            ...
            dispose();
        } else if (birdLaunched && birdQueue.isEmpty() && birdPosition.y < GROUND_LEVEL + birdRadius) {
            gameOver = true;
            game.setScreen(new WinScreen3(game));
            dispose();
        }
    ...
    private void saveGame() {
        GameSavelev3.saveGame(this);
    }

    private void loadGame() {
        GameSavelev3.loadGame(this);
    }
